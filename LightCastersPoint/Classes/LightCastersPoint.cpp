...
void LightCastersPoint::initProgram()
{
...
void LightCastersPoint::onTouchBegan(float x, float y)
{
    m_lastX = x;
    m_lastY = y;
}

void LightCastersPoint::onTouchMove(float x, float y)
{
    if (m_firstMouse)
    {
        m_lastX = x;
...
void LightCastersPoint::onTouchEnd(float x, float y)
{
    
}

void LightCastersPoint::onMouseScroll(double x, double y)
{
    m_camera.ProcessMouseScroll(y);
}

void LightCastersPoint::onKeyPress(int key, int scancode, int action, int mods)
{
    if(key == GLFW_KEY_W)
    {
