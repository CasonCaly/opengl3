...
{
    this->createPrograme("Shaders/Materials.vert", "Shaders/Materials.frag", m_lightingShader);
...
void Materials::render()
{
    ...
    m_lightingShader.getUniform("light.ambient")->vector3fv(1, &ambientColor[0]);
    m_lightingShader.getUniform("light.diffuse")->vector3fv(1, &diffuseColor[0]);
    m_lightingShader.getUniform("light.specular")->value3f(1.0f, 1.0f, 1.0f);
    
    //material properties
    m_lightingShader.getUniform("material.ambient")->value3f(1.0f, 0.5f, 0.31f);
    m_lightingShader.getUniform("material.diffuse")->value3f(1.0f, 0.5f, 0.31f);
...
void Materials::onTouchBegan(float x, float y)
{
    m_lastX = x;
    m_lastY = y;
}

void Materials::onTouchMove(float x, float y)
{
    if (m_firstMouse)
    {
        m_lastX = x;
...
void Materials::onTouchEnd(float x, float y)
{
    
}

void Materials::onMouseScroll(double x, double y)
{
    m_camera.ProcessMouseScroll(y);
}

void Materials::onKeyPress(int key, int scancode, int action, int mods)
{
    if(key == GLFW_KEY_W)
    {
