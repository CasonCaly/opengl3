...
BasicLightingSpecular::BasicLightingSpecular():
...
void BasicLightingSpecular::initProgram()
{
...
void BasicLightingSpecular::render()
{
    ...
    m_lightingShader.getUniform("viewPos")->value3f(m_camera.Position.x, m_camera.Position.y, m_camera.Position.z);
...
void BasicLightingSpecular::onTouchBegan(float x, float y)
{
    m_lastX = x;
    m_lastY = y;
}

void BasicLightingSpecular::onTouchMove(float x, float y)
{
    if (m_firstMouse)
    {
        m_lastX = x;
...
void BasicLightingSpecular::onTouchEnd(float x, float y)
{
    
}

void BasicLightingSpecular::onMouseScroll(double x, double y)
{
    m_camera.ProcessMouseScroll(y);
}

void BasicLightingSpecular::onKeyPress(int key, int scancode, int action, int mods)
{
    if(key == GLFW_KEY_W)
    {
