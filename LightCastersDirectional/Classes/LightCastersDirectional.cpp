...
void LightCastersDirectional::initProgram()
{
...
void LightCastersDirectional::render()
{
    ...
    //light
    m_lightingShader.use();
    m_lightingShader.getUniform("light.direction")->value3f(-0.2f, -1.0f, -0.3f);
...
void LightCastersDirectional::onTouchBegan(float x, float y)
{
    m_lastX = x;
    m_lastY = y;
}

void LightCastersDirectional::onTouchMove(float x, float y)
{
    if (m_firstMouse)
    {
        m_lastX = x;
...
void LightCastersDirectional::onTouchEnd(float x, float y)
{
    
}

void LightCastersDirectional::onMouseScroll(double x, double y)
{
    m_camera.ProcessMouseScroll(y);
}

void LightCastersDirectional::onKeyPress(int key, int scancode, int action, int mods)
{
    if(key == GLFW_KEY_W)
    {
